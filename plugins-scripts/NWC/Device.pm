...
sub new {
  ...
  if (! ($self->opts->hostname || $self->opts->snmpwalk)) {
    ...
    if (! $NWC::Device::plugin->check_messages() || $self->opts->servertype) {
      ...
      if ($self->{productname} =~ /Rittal/i) {
        bless $self, 'NWC::Rittal';
        $self->debug('using NWC::Rittal');
      } else {
...
sub make_symbolic {
  ...
  foreach my $index (@{$indices}) {
    ...
    my $mo = {};
    my $idx = $withindex ? join('.', @{$index}) : ''; # index can be multi-level
    foreach my $symoid
        (keys %{$NWC::Device::mibs_and_oids->{$mib}}) {
      my $oid = $NWC::Device::mibs_and_oids->{$mib}->{$symoid};
      if (ref($oid) ne 'HASH') {
        my $fulloid = $withindex ? $oid . '.'.$idx : $oid;
